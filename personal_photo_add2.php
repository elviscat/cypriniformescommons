<?php 
	...
	if(isset($_POST['submit_b']) && $_POST['post_from_form'] === '1'){
		if ($_FILES["file"]["error"] > 0 ){
			...
			$media_type = $_FILES['media_content']['type'];
			//$_FILES["upload"]["tmp_name"]
			$fd = fopen($_FILES['media_content']['tmp_name'], "rb"); 
			$size = $_FILES['media_content']['size']; 
			$block = fread($fd, $size);//media_content
			...
			$sql_update = "UPDATE `participants` SET photo = '".$block."' WHERE id = '".$id."'";
